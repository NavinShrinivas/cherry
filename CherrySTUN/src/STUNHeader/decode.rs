...
impl STUNDecode for STUNHeader {
    fn decode(&self) -> Result<Vec<u8>, crate::STUNError::error::STUNError> {
        let bin: Vec<u8> = Vec::new();
        let mut header_cursor = Cursor::new(bin);
        let message_type = (self.message_method as u16 & 0b0011_1110_1110_1111)
            | (self.message_class as u16 & 0b0000_0001_0001_000);
        ...
        match header_cursor.write_u16::<NetworkEndian>(message_type) {
            ...
            Err(e) => {
                return Err(STUNError::new(
                    ...
                    e.to_string() + "Error writing message type to binary format while decoding STUNHeader.",
                ))
            }
        }
        ...
        match header_cursor.write_u16::<NetworkEndian>(message_len) {
            ...
            Err(e) => {
                return Err(STUNError::new(
                    STUNStep::STUNDecode,
                    STUNErrorType::WriteError,
                    e.to_string() + "Error writing message length to binary format while decoding STUNHeader.",
                ))
            }
        }
        ...
        match header_cursor.write_u32::<NetworkEndian>(magic_num) {
            ...
            Err(e) => {
                return Err(STUNError::new(
                    STUNStep::STUNDecode,
                    STUNErrorType::WriteError,
                    e.to_string() + "Error writing magic number to binary format while decoding STUNHeader.",
                ))
            }
        }
...
mod test {
    ...
    fn test_stun_header_decode() {
        ...
        let mut stun_binding_request_header = STUNHeader::new(
            STUNMessageClass::Request,
            STUNMessageMethod::Binding,
            Some(fixtures::EXAMPLE_STUN_REQUEST_TRANSACTION_ID),
        );
        stun_binding_request_header.increment_message_length(88);
        assert_eq!(
            stun_binding_request_header.decode().unwrap(),
